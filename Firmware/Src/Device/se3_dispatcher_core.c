...
uint16_t dispatcher_RSA(uint16_t req_size, const uint8_t* req, uint16_t* resp_size, uint8_t* resp){
    uint8_t operation = 0; // this type must be consistent with what is specified in the host side!
    if (!login_struct.y) {
        SE3_TRACE(("[dispatcher_RSA] not logged in\n"));
        return SE3_ERR_ACCESS;
    }
    memcpy(&operation, req, 1);
    const uint8_t *req_no_op = req + 1;
    const uint16_t req_size_no_op = req_size - 1;

    switch (operation) {
		case SE3_RSA_KEYADD:
			return se3_rsa_keyadd((uint16_t)req_size_no_op, (uint8_t*)req_no_op, resp_size, resp);
		case SE3_RSA_KEYGEN:
			return se3_rsa_keygen(req_size_no_op, req_no_op, resp_size, resp);
		case SE3_RSA_KEYDEL:
			return se3_rsa_keydel(req_size_no_op, req_no_op, resp_size, resp);
		case SE3_RSA_KEYGET:
			return se3_rsa_keyget(req_size_no_op, req_no_op, resp_size, resp);
		case SE3_RSA_KEYFIND:
			return se3_rsa_keyfind(req_size_no_op, req_no_op, resp_size, resp);
		case SE3_RSA_ENCRYPT: // go to se3_rsa_operate()
		case SE3_RSA_DECRYPT: // go to se3_rsa_operate()
		case SE3_RSA_SIGN:    // go to se3_rsa_operate()
		case SE3_RSA_VERIFY:
			return se3_rsa_operate(req_size, (uint8_t*)req, resp_size, resp);
		case SE3_RSA_X509_GEN:
			return se3_rsa_x509_cert_gen(req_size_no_op, req_no_op, resp_size, resp);
		case SE3_RSA_X509_GET:
			return se3_rsa_x509_cert_get(req_size_no_op, req_no_op, resp_size, resp);
		case SE3_RSA_X509_DELETE:
			return se3_rsa_x509_cert_delete(req_size_no_op, req_no_op, resp_size, resp);
		case SE3_RSA_X509_LIST:
			return se3_rsa_x509_cert_list(req_size_no_op, req_no_op, resp_size, resp);
		case SE3_RSA_X509_FIND:
			return se3_rsa_x509_cert_find(req_size_no_op, req_no_op, resp_size, resp);
		default:
			SE3_TRACE(("[dispatcher_RSA] invalid operation\n"));
