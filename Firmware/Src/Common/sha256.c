...
int32_t B5_Sha256 (const uint8_t *data, int32_t dataLen, uint8_t *rDigest)
{
	int32_t ret = B5_SHA256_RES_OK;
	B5_tSha256Ctx ctx;

	if ((ret = B5_Sha256_Init(&ctx)) != B5_SHA256_RES_OK)
		goto exit;

	if ((ret = B5_Sha256_Update(&ctx, data, dataLen)) != B5_SHA256_RES_OK)
		goto exit;

	ret = B5_Sha256_Finit(&ctx, rDigest);
exit:
	return ret;
}
