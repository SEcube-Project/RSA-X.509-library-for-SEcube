...
uint16_t L0::L0TX(uint16_t cmd, uint16_t cmdFlags, uint16_t len, const uint8_t* data, uint32_t timeout) {
#if SE3_CONF_CRC
	uint16_t crc;
#endif
	...
	if (!L0Support::Se3Write(this->base.GetDeviceRequest(), this->base.GetDeviceFile(), 0, nBlocks, timeout))
		return L0ErrorCodes::Error::COMMUNICATION;

	return L0ErrorCodes::Error::OK;
}

uint16_t L0::L0RX(uint16_t* respStatus, uint16_t* respLen, uint8_t* respData, uint32_t timeout) {
#if SE3_CONF_CRC
	uint16_t crc
#endif
	...
	bool success = true;
	uint16_t u16tmp;
	uint64_t deadline = L0Support::Se3Deadline(timeout);
	...
	while (!ready) {
		...
		if (!L0Support::Se3Read(this->base.GetDeviceResponse(), this->base.GetDeviceFile(), 0, 1, timeout)) {
			success = false;
			break;
		}
	...
	if (nBlocks > 1)
		if (!L0Support::Se3Read(this->base.GetDeviceResponse() + L0Communication::Parameter::COMM_BLOCK, this->base.GetDeviceFile(), 1, nBlocks - 1, timeout))
			return L0ErrorCodes::Error::COMMUNICATION;
...
void L0::Open() {
	Open(this->base.GetDevicePtr());
}

void L0::Open(uint8_t devPtr) {
	se3File hFile;
...
void L0::Close() {
	Close(this->base.GetDevicePtr());
}

void L0::Close(uint8_t devPtr) {
	L0NoDeviceException ptrExc;
...
void L0::L0TXRX(uint16_t reqCmd, uint16_t reqCmdFlags, uint16_t reqLen, const uint8_t* reqData, uint16_t* respStatus, uint16_t* respLen, uint8_t* respData, uint32_t timeout) {
	...
	error = L0TX(reqCmd, reqCmdFlags, reqLen, reqData, timeout);

	if (error != L0ErrorCodes::Error::OK){
		throw txExc;
	}

	error = L0RX(respStatus, respLen, respData, timeout);
...
uint16_t L0::Echo(const uint8_t* dataIn, uint16_t dataInLen, uint8_t* dataOut) {
