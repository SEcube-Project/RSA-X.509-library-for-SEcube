...
private:
	...
	void DiscoverInit() override ;
	bool DiscoverNext() override ;
	//COMMUNICATION
	bool Se3Open(uint64_t deadline, se3File* phFile, se3DiscoverInfo* disco) override ;
	uint16_t L0TX(uint16_t cmd, uint16_t cmdFlags, uint16_t len, const uint8_t* data, uint32_t timeout = SE3_TIMEOUT) override ;
	uint16_t L0RX(uint16_t* respStatus, uint16_t* respLen, uint8_t* respData, uint32_t timeout = SE3_TIMEOUT) override ;
public:
	L0();
	~L0();

	//COMMODITIES
	bool DiscoverSerialNo(uint8_t* serialNo) override ;

	//COMMUNICATION
	void Open() override ;
	void Close() override ;
	void Open(uint8_t devPtr) override ;
	void Close(uint8_t devPtr) override ;
	void L0TXRX(uint16_t reqCmd, uint16_t reqCmdFlags, uint16_t reqLen, const uint8_t* reqData, uint16_t* respStatus, uint16_t* respLen, uint8_t* respData, uint32_t timeout = SE3_TIMEOUT) override ;
	uint16_t Echo(const uint8_t* dataIn, uint16_t dataInLen, uint8_t* dataOut) override ;
