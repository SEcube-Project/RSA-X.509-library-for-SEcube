...
private:
	...
	virtual uint16_t L0TX(uint16_t cmd, uint16_t cmdFlags, uint16_t len, const uint8_t* data, uint32_t timeout = SE3_TIMEOUT) = 0;
	virtual uint16_t L0RX(uint16_t* respStatus, uint16_t* respLen, uint8_t* respData, uint32_t timeout = SE3_TIMEOUT) = 0;
public:
	...
	virtual void Open(uint8_t devPtr) = 0;
	/** @brief For internal usage of L0 and L1 levels. */
	virtual void Close(uint8_t devPtr) = 0;
	/** @brief Send command to SEcube, wait for reply, read reply (low level, used also by L1TXRX()). */
	virtual void L0TXRX(uint16_t reqCmd, uint16_t reqCmdFlags, uint16_t reqLen, const uint8_t* reqData, uint16_t* respStatus, uint16_t* respLen, uint8_t* respData, uint32_t timeout = SE3_TIMEOUT) = 0;
	/** @brief The SEcube echoes back any data it receives. */
	virtual uint16_t Echo(const uint8_t* dataIn, uint16_t dataInLen, uint8_t* dataOut) = 0;
};
