...
void L1::Get_algorithms(std::vector<se3Algo>& algorithmsArray) {
...
L1::~L1() {
	if(this->index != 255){ // this is used by SEkey
		...
		this->Close(this->index); // added to remove memory leakage of se3file
		...
		for (uint8_t i = 0; i < this->GetNumberDevices(); i++) {
			...
			this->Close(i); // added to remove memory leakage of se3file
		}
	}
}
...
void L1::TXRXData(uint16_t cmd, uint16_t reqLen, uint16_t cmdFlags, uint16_t* respLen, uint32_t timeout) {
	//SET THE HEADERS
	...
	while(!dataSent) {
		try {
			L0::L0TXRX(L0Commands::Command::L1_CMD0, cmdFlags, req0Len, this->base.GetSessionBuffer(), &respStatus, &resp0Len, this->base.GetSessionBuffer(), timeout);
			dataSent = true;
		}
