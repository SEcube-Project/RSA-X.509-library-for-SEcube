...
void L1::Crypto_init(uint16_t algorithm, uint16_t mode, uint32_t keyId, uint32_t& sessId) {
...
void L1::Crypto_update(uint32_t sessId, uint16_t flags, uint16_t data1Len, uint8_t* data1, uint16_t data2Len, uint8_t* data2, uint16_t* dataOutLen, uint8_t* dataOut) {
...
void L1::Symm_enc(size_t plaintext_size, std::shared_ptr<uint8_t[]> plaintext, SEcube_ciphertext& encrypted_data, uint16_t algorithm, uint16_t algorithm_mode, uint32_t key_id) {
	...
	try {
		Crypto_init(algorithm, algorithm_mode | CryptoInitialisation::Direction::ENCRYPT, key_id, encSessId);
		...
		if(algorithm_mode == CryptoInitialisation::Modes::CTR){
			enum{
				/* we need this new value because, in CTR mode, the Crypto_update will be called passing both the cleartext and the counter,
			...
			if(algorithm == L1Algorithms::Algorithms::AES_HMACSHA256){ // AES + HMAC-SHA256
				...
				Crypto_update(encSessId, L1Crypto::UpdateFlags::SETNONCE, 32, nonce_key_derivation_hmac_sha256, 0, nullptr, nullptr, nullptr); // set nonce for HMAC-SHA256 key derivation
			...
			do {
				if(total_size - curr_chunk){ // still in the middle of data
					if(algorithm == L1Algorithms::Algorithms::AES_HMACSHA256){ // AES with HMAC-SHA256
						Crypto_update(encSessId, L1Crypto::UpdateFlags::RESET, B5_AES_BLK_SIZE, ctr_nonce, curr_chunk, decrypted, &curr_len, encrypted);
					...
					if(algorithm == L1Algorithms::Algorithms::AES_HMACSHA256){ // AES with HMAC-SHA256
						Crypto_update(encSessId, L1Crypto::UpdateFlags::RESET | L1Crypto::UpdateFlags::AUTH | L1Crypto::UpdateFlags::FINIT, B5_AES_BLK_SIZE, ctr_nonce, curr_chunk, decrypted, &curr_len, encrypted);
					} else { // standard AES without HMAC-SHA256
						Crypto_update(encSessId, L1Crypto::UpdateFlags::FINIT, B5_AES_BLK_SIZE, ctr_nonce, curr_chunk, decrypted, &curr_len, encrypted);
					}
				}
			...
			if((algorithm_mode == CryptoInitialisation::Modes::CBC) ||
			   ...
			   (algorithm_mode == CryptoInitialisation::Modes::OFB)){
				...
				Crypto_update(encSessId, L1Crypto::UpdateFlags::SET_IV, B5_AES_BLK_SIZE, aes_initialization_vector, 0, nullptr, nullptr, nullptr); // set IV
			...
			do {
				if(total_size - curr_chunk){ // still in the middle of data
					if(algorithm == L1Algorithms::Algorithms::AES_HMACSHA256){ // AES with HMAC-SHA256
						Crypto_update(encSessId, 0, 0, nullptr, curr_chunk, decrypted, &curr_len, encrypted);
					} else { // standard AES without HMAC-SHA256
						Crypto_update(encSessId, 0, 0, nullptr, curr_chunk, decrypted, &curr_len, encrypted);
					}
				}
					...
					if(algorithm == L1Algorithms::Algorithms::AES_HMACSHA256){ // AES with HMAC-SHA256
						Crypto_update(encSessId, L1Crypto::UpdateFlags::RESET | L1Crypto::UpdateFlags::AUTH | L1Crypto::UpdateFlags::FINIT, 0, nullptr, curr_chunk, decrypted, &curr_len, encrypted);
					} else { // standard AES without HMAC-SHA256
						Crypto_update(encSessId, L1Crypto::UpdateFlags::FINIT, 0, nullptr, curr_chunk, decrypted, &curr_len, encrypted);
					}
				}
				total_size -= curr_chunk; // decrement total number of bytes
				decrypted += curr_chunk; // move on pointer of plaintext
...
void L1::Symm_dec(SEcube_ciphertext& encrypted_data, size_t& plaintext_size, std::shared_ptr<uint8_t[]>& plaintext) {
	...
	try {
		Crypto_init(algorithm, algorithm_mode | CryptoInitialisation::Direction::DECRYPT, key_id, encSessId);
		decrypted_data  = make_unique<uint8_t[]>(encrypted_data.ciphertext_size); // by default allocated for simple AES
		if(algorithm_mode == CryptoInitialisation::Modes::CTR){
			enum{
				/* we need this new value because, in some cases, the Crypto_update will be called passing both the cleartext and the nonce,
				 * so we have to consider also the size of the nonce when sending the cryptoupdate. */
				DATAIN_NEW = L1Crypto::UpdateSize::DATAIN - B5_AES_BLK_SIZE
			};
			...
			if(algorithm == L1Algorithms::Algorithms::AES_HMACSHA256){ // AES + HMAC-SHA256
				...
				Crypto_update(encSessId, L1Crypto::UpdateFlags::SETNONCE, 32, encrypted_data.digest_nonce.data(), 0, nullptr, nullptr, nullptr); // set nonce for HMAC-SHA256 key derivation
			}
			...
			do {
				if(enc_size - curr_chunk){ // still in the middle of data
					if(algorithm == L1Algorithms::Algorithms::AES_HMACSHA256){ // AES with HMAC-SHA256
						Crypto_update(encSessId, L1Crypto::UpdateFlags::RESET, B5_AES_BLK_SIZE, ctr_nonce, curr_chunk, encrypted, &curr_len, decrypted);
					} else { // standard AES without HMAC-SHA256
						Crypto_update(encSessId, L1Crypto::UpdateFlags::RESET, B5_AES_BLK_SIZE, ctr_nonce, curr_chunk, encrypted, &curr_len, decrypted);
					}
				}
					...
					if(algorithm == L1Algorithms::Algorithms::AES_HMACSHA256){ // AES with HMAC-SHA256
						Crypto_update(encSessId, L1Crypto::UpdateFlags::RESET | L1Crypto::UpdateFlags::AUTH | L1Crypto::UpdateFlags::FINIT, B5_AES_BLK_SIZE, ctr_nonce, curr_chunk, encrypted, &curr_len, decrypted);
					} else { // standard AES without HMAC-SHA256
						Crypto_update(encSessId, L1Crypto::UpdateFlags::FINIT, B5_AES_BLK_SIZE, ctr_nonce, curr_chunk, encrypted, &curr_len, decrypted);
					}
				}
				ctr_counter++; // increment counter and concatenate it with the nonce
				memcpy(ctr_nonce+8, &ctr_counter, 8);
			...
			if((algorithm_mode == CryptoInitialisation::Modes::CBC) ||
			   (algorithm_mode == CryptoInitialisation::Modes::CFB) ||
			   (algorithm_mode == CryptoInitialisation::Modes::OFB)){
				Crypto_update(encSessId, L1Crypto::UpdateFlags::SET_IV, B5_AES_BLK_SIZE, encrypted_data.initialization_vector.data(), 0, nullptr, nullptr, nullptr); // set IV
			}
			uint8_t *decrypted = decrypted_data.get(); // alias for plaintext
			uint8_t *encrypted = encrypted_data.ciphertext.get(); // alias for ciphertext
			do {
				if(enc_size - curr_chunk){ // still in the middle of data
					if(algorithm == L1Algorithms::Algorithms::AES_HMACSHA256){ // AES with HMAC-SHA256
						Crypto_update(encSessId, 0, 0, nullptr, curr_chunk, encrypted, &curr_len, decrypted);
					} else { // standard AES without HMAC-SHA256
						Crypto_update(encSessId, 0, 0, nullptr, curr_chunk, encrypted, &curr_len, decrypted);
					}
				}
					...
					if(algorithm == L1Algorithms::Algorithms::AES_HMACSHA256){ // AES with HMAC-SHA256
						Crypto_update(encSessId, L1Crypto::UpdateFlags::RESET | L1Crypto::UpdateFlags::AUTH | L1Crypto::UpdateFlags::FINIT, 0, nullptr, curr_chunk, encrypted, &curr_len, decrypted);
					} else { // standard AES without HMAC-SHA256
						Crypto_update(encSessId, L1Crypto::UpdateFlags::FINIT, 0, nullptr, curr_chunk, encrypted, &curr_len, decrypted);
					}
				}
				enc_size -= curr_chunk; // decrement total number of bytes
				decrypted += curr_chunk; // move on pointer of plaintext
...
void L1::Digest(size_t input_size, std::shared_ptr<uint8_t[]> input_data, SEcube_digest& digest) {
	...
	try {
		switch(digest.algorithm){
			case L1Algorithms::Algorithms::HMACSHA256:
				Crypto_init(digest.algorithm, 0, digest.key_id, encSessId);
				if(digest.usenonce){
					Crypto_update(encSessId, L1Crypto::UpdateFlags::SETNONCE, 32, digest.digest_nonce.data(), 0, nullptr, nullptr, nullptr);
				} else {
					uint8_t nonce_key_derivation_hmac_sha256[32];
					memset(nonce_key_derivation_hmac_sha256, 0, 32);
					L0Support::Se3Rand(32, nonce_key_derivation_hmac_sha256);
					Crypto_update(encSessId, L1Crypto::UpdateFlags::SETNONCE, 32, nonce_key_derivation_hmac_sha256, 0, nullptr, nullptr, nullptr);
			...
			case L1Algorithms::Algorithms::SHA256:
				Crypto_init(digest.algorithm, 0, L1Key::Id::NULL_ID, encSessId);
				break;
			default:
				throw digestExc;
		}
		...
		do {
			if(input_size - curr_chunk){ // still in the middle of data
				Crypto_update(encSessId, 0, curr_chunk, input, 0, nullptr, &curr_len, output);
			}
			else{ // last chunk of data
				Crypto_update(encSessId, L1Crypto::UpdateFlags::FINIT, curr_chunk, input, 0, nullptr, &curr_len, output);
			}
...
void L1::Edit_symm_key(se3SymmKey& key, uint16_t op){
	L1KeyEditException keyEditExc;
	uint16_t dataLen = 0;
	uint16_t respLen = 0;
    if((key.id == L1Key::Id::NULL_ID) ||
       (key.id == L1Key::Id::ZERO_ID) ||
	   (key.id >= L1Key::Id::SEKEY_ID_BEGIN && key.id <= L1Key::Id::SEKEY_ID_END) ||
	   (key.id >= L1Key::Id::RESERVED_ID_SEKEY_BEGIN && key.id <= L1Key::Id::RESERVED_ID_SEKEY_END)){
    	throw keyEditExc;
    }
    if(op != L1Commands::KeyOpEdit::SE3_KEY_OP_GENERATE_SYMM &&
       op != L1Commands::KeyOpEdit::SE3_KEY_OP_INJECT_SYMM &&
	   op != L1Commands::KeyOpEdit::SE3_KEY_OP_DELETE){
    	throw keyEditExc;
    }
	this->base.FillSessionBuffer((uint8_t*)&op, L1Response::Offset::DATA + L1Request::KeyOffset::OP, 2);
	dataLen += 2;
	this->base.FillSessionBuffer((uint8_t*)&(key.id), L1Response::Offset::DATA + L1Request::KeyOffset::ID, 4); // ID always required
	dataLen += 4;
	this->base.FillSessionBuffer((uint8_t*)&(key.length), L1Response::Offset::DATA + L1Request::KeyOffset::DATA_LEN, 2);
	dataLen += 2;
	if(op == L1Commands::KeyOpEdit::SE3_KEY_OP_INJECT_SYMM){
		if(key.value == nullptr){
			throw keyEditExc;
		}
		this->base.FillSessionBuffer(key.value, L1Response::Offset::DATA + L1Request::KeyOffset::DATA, key.length);
		dataLen += key.length;
	}
	try {
...
void L1::List_symm_key(std::vector<std::pair<uint32_t, uint16_t>>& keylist){
...
void L1::Find_symm_key(uint32_t keyId, bool& found) {
