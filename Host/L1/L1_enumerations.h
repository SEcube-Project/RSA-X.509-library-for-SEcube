...
namespace L1Commands {
	...
	/** These codes are used to select a specific sub-operation identified by L1Commands::Codes::RSA. They are used only to implement RSA
	 * algorithm required by SEkey. */
	struct RSA_Options {
		enum{
			SE3_RSA_KEYADD = 0, 
			SE3_RSA_KEYGEN = 1,
	...
	struct KeyOpEdit {
		enum {
		    SE3_KEY_OP_INJECT_SYMM = 1,     /**< Inject symmetric key into SEcube. */
		    SE3_KEY_OP_DELETE = 2,			/**< Delete a key from the SEcube. */
		    SE3_KEY_OP_GENERATE_SYMM = 3,	/**< Generate symmetric key inside the SEcube (the key is not exposed outside). */
			SE3_KEY_OP_INJECT_ASYMM = 4,	/**< Inject asymmetric key into SEcube. */
			SE3_KEY_OP_GENERATE_ASYMM = 5,	/**< Generate asymmetric key inside the SEcube (the key is not exposed outside). */
		};
	};

	enum CertOpEdit {
		SE3_CERT_OP_STORE = 0,		/**< Store a certificate inside the SEcube. */
		SE3_CERT_OP_DELETE = 1,		/**< Delete a certificate from the SEcube. */
	};
}
...
namespace L1Algorithms {
	...
	struct Algorithms {
		enum {
			...
			RSA = 4,			/**< RSA */
			ALGORITHM_MAX = 5	/**< Value required by SEfile */
		};
	};
}
