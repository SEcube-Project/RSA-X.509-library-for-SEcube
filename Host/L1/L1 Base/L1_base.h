...
struct se3AsymmKey{
	uint32_t id; // ID of the key (0 to UINT32_MAX - 1)
	uint16_t length; // length of the key (bytes!)
	std::unique_ptr<uint8_t[]> N; // public modulus
	std::unique_ptr<uint8_t[]> E; // public exponent
	std::unique_ptr<uint8_t[]> D; // private exponent (secret)
	uint8_t type;
};
...
struct se3SymmKey {
	uint32_t id; // ID of the key (0 to UINT32_MAX - 1)
	uint16_t length; // length of the key (bytes!)
	uint8_t* value; // actual key value
};
